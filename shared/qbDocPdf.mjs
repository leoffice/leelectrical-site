// QuickBooks-clone invoice/estimate PDF — a pure byte-writer port of the old
// server pdfkit template (netlify/functions/lib/le-invoice-suite/qb-pdf.js).
...
//
// This module lives in shared/ because BOTH runtimes render it:
//   - the browser, via pro-src/src/lib/qbInvoicePdf.js (wraps the bytes in a Blob)
//   - Cloudflare Pages Functions, via netlify/functions/lib/docGenerate.mjs
// so a customer's invoice can be rendered server-side without the office Mac.
// It must therefore stay free of DOM, Node and tenant-state imports: everything
// environment-specific is INJECTED through `data` (see buildQbDocPdfBytes).
import { LE_LOGO_JPEG, leLogoJpegBytes } from "./leLogoJpeg.mjs";

// Pure layout constants — inlined from pro-src brand.js, which pulls in live
// tenant state and so cannot be imported from a Worker.
const POWERED_BY_LE_PDF_COLOR = [75 / 255, 85 / 255, 99 / 255];
const POWERED_BY_LE_PDF_SIZE = 8.5;
...
/**
 * Render a QuickBooks-clone invoice/estimate to raw PDF bytes (Uint8Array).
 *
 * `data` is the mapJobToQbDocData shape: { docType,
 * company:{name,addressLines,phone,email}, docNumber, date, dueDate, terms,
 * billTo:{name,addressLines}, customFields:[{label,value}], serviceDate,
 * lines:[{description,rate,qty,amount}], subtotal, tax, total, payment,
 * messageLines:[], footerLines:[], showAcceptance, payUrl }
 *
 * plus three optional INJECTED branding fields, since this module cannot read
 * live tenant state (see the header note):
 *   poweredBy — product mark under the footer; omitted entirely when falsy
 *   tenant    — { name, email }, only used to build fallback messageLines
 *   profile   — { zelleInstructions }, likewise
 * Callers that pass `messageLines` never need `tenant`/`profile`.
 *
 * Returns bytes rather than a Blob so Workers and the browser share one path;
 * the browser wrapper adds the Blob.
 */
export function buildQbDocPdfBytes(data) {
  const pg = Page();
  ...
  if (data.poweredBy) {
    pg.center(data.poweredBy, fy + 14, {
