...
/**
 * Translate a Cloudflare state request into the shared merge body.
 * PATCH { id, patch, base? } becomes a one-key patch. POST is used as-is
 * (full ov, { op:"patch", ov }, or { op:"delete" }).
 */
export function stateWriteBody(method, body) {
  const req = isPlainObject(body) ? body : {};
  if (String(method || "").toUpperCase() !== "PATCH") return req;
  if (req.id == null) return { error: "id required" };
  const id = String(req.id);
  let base;
  if (isPlainObject(req.base) && isPlainObject(req.base[id])) base = { [id]: req.base[id] };
  else if (isPlainObject(req.base) && (req.base._savedAt != null || req.base._version != null)) {
    const stamp = {};
    if (req.base._savedAt != null) stamp._savedAt = req.base._savedAt;
    if (req.base._version != null) stamp._version = req.base._version;
    base = { [id]: stamp };
  }
  return { op: "patch", ov: { [id]: req.patch == null ? {} : req.patch }, base };
}
