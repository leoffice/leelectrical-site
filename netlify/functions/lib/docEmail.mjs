...
import { base64FromBytes } from "./base64.mjs";
import { LOGO_BYTES } from "./le-invoice-suite/assets-embedded.mjs";
...
export async function sendDocEmail({ job, kind = "invoice", to, includePaymentLink = true }) {
  ...
  // Workers-safe: embedded logo bytes + base64 helpers (no Node Buffer/fs).
  const pdfB64 = base64FromBytes(gen.pdfBuffer);
  const logoB64 = base64FromBytes(LOGO_BYTES);
  ...
  const payload = {
    ...
    attachments: [
      ...
      { filename: "logo.png", content: logoB64, content_id: "companylogo" },
    ],
  };
