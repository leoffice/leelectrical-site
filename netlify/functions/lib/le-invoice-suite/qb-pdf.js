...
// On the Cloudflare Workers bundle there is no __dirname. These fs paths are
// only a Node fallback — the Workers path passes font/logo bytes directly —
// so a placeholder is safe and keeps module evaluation from throwing.
const __ASSET_DIR = (typeof __dirname !== 'undefined') ? __dirname : '.';
...
const STYLE = {
  ...
  fonts: {
    regular: path.join(__ASSET_DIR, 'fonts', 'LiberationSans-Regular.ttf'),
    bold:    path.join(__ASSET_DIR, 'fonts', 'LiberationSans-Bold.ttf'),
  },

  logo: {
    path: path.join(__ASSET_DIR, 'assets', 'logo.png'),
...
function generateDocument(data, outPath) {
  return new Promise((resolve, reject) => {
    ...
    const doc = new PDFDocument({
      ...
      bufferPages: true,
      autoFirstPage: true,
      // font:false skips pdfkit's eager Helvetica AFM load (fs + __dirname),
      // which is unavailable on the Workers runtime. We only ever use the
      // registered TTF fonts below, so the standard fonts are never needed.
      font: false,
    ...
    // Workers-safe: prefer embedded font bytes (data.fontRegular/fontBold),
    // fall back to the bundled .ttf file paths when running under Node.
    doc.registerFont('reg', data.fontRegular || S.fonts.regular);
    doc.registerFont('bold', data.fontBold || S.fonts.bold);
    // Set an active font immediately (font:false left _font unset).
    doc.font('reg');
    ...
    const drawPageHeader = () => {
      ...
      // Workers-safe: prefer embedded logo bytes, fall back to the file path.
      if (data.logoBytes) {
        doc.image(data.logoBytes, S.logo.x, S.logo.y, { fit: [S.logo.w, S.logo.h], align: 'center' });
      } else {
