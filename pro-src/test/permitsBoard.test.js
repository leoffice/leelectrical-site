...
describe("computePermitBackfill — combined Con Ed + City, one unified permits[]", () => {
  it("merges both agencies into a single permits patch (no clobber) + idempotent", () => {
    ...
    // Idempotent: persist full patch (permits + paperwork + status + pipe valve) then re-plan → no-op.
    const p0 = plan[0].patch;
    const persisted = JOB("J-1", {
      ...(p0.permitTracker != null ? { permitTracker: p0.permitTracker } : {}),
      permits: p0.permits,
      paperwork: p0.paperwork,
      status: {
        ...(JOB("J-1").status || {}),
        ...(p0.status || {}),
      },
