...
describe("ETag is tenant-namespaced — a 304 can never cross tenants", () => {
  const gid = (token, inm) => {
    const headers = { Authorization: `Bearer ${token}` };
    if (inm) headers["if-none-match"] = inm;
    return new Request(STATE_URL, { method: "GET", headers });
  };

  it("B sending A's ETag gets 200 with its OWN data, never a 304 to A's body", async () => {
    await saveOv("tokA", { "job-A1": { customer: "Acme A", invoiceNo: "A-100" } });
    const aRes = await stateHandler(gid("tokA"));
    const aEtag = aRes.headers.get("etag");
    expect(aEtag).toBeTruthy();
    // Same URL, A's ETag, but B's token → tenant-namespaced tag differs → no 304.
    const bRes = await stateHandler(gid("tokB", aEtag));
    expect(bRes.status).toBe(200);
    const bBody = await asJson(bRes);
    expect(bBody.ov["job-A1"]).toBeUndefined();
  });

  it("same tenant with a matching ETag still gets a 304 (perf preserved)", async () => {
    await saveOv("tokA", { "job-A1": { customer: "Acme A" } });
    const first = await stateHandler(gid("tokA"));
    const etag = first.headers.get("etag");
    const second = await stateHandler(gid("tokA", etag));
