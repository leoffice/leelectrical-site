...
describe("letterhead PDF", () => {
  ...
  it("renders correctly with Levi's real prod data (Lic #-prefixed license, no owners, lowercase address)", () => {
    setActiveTenantConfig({
      profile: {
        companyName: "BLZ Electric Inc.",
        shortName: "BLZ Electric",
        ...
        phone: "(718) 594-1850",
        email: "Office@LeElectrical.us",
        license: "Lic #11212", // stored WITH prefix, as in prod
        website: "leelectrical.us",
        // no owners[] — prod profile may not have them
      },
      branding: { companyName: "BLZ Electric Inc." },
      internal: true,
    });
    ...
    draft.status = "approved";
    // Lowercase job address is title-cased everywhere.
    expect(draft.siteAddress).toBe("1254 Sterling Pl Brooklyn");
    const bytes = buildLetterheadPdf({ draft });
    let asText = "";
    for (let i = 0; i < bytes.length; i++) asText += String.fromCharCode(bytes[i]);
    // License prints once, no "Lic. Lic #" doubling.
    expect(asText).toMatch(/License #11212/);
    expect(asText).not.toMatch(/Lic\.? #?\s*Lic/i);
    // Signer line guaranteed even without owners[]: Levi Kumer + President.
    expect(asText).toMatch(/Levi Kumer/);
    expect(asText).toMatch(/President/);
    expect(asText).toMatch(/\/ImSig/);
    // Single salutation, no legacy RE/Site duplication for redesigned types.
    expect(asText.match(/To Whom It May Concern/g)?.length).toBe(1);
    expect(asText).not.toMatch(/Site: /);
  });

  it("general letters print the salutation once (no doubled To Whom It May Concern)", () => {
    const type = LETTER_TYPES.find((t) => t.id === "general");
    const draft = createLetterDraft({
      type,
      job: { serviceAddress: "1 A St" },
      answers: { recipient: "To Whom It May Concern", body: "Hello." },
    });
    draft.status = "approved";
    const bytes = buildLetterheadPdf({ draft });
    let asText = "";
