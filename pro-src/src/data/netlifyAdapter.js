...
export function createNetlifyAdapter() {
  ...
  let cachedOv = null;
  // Stamps from our own successful writes, so a later save can send a base
  // version without re-downloading the overlay.
  ...
  return {
    ...
    async appendWorkDescriptionPolishFeedback(entry) {
      ...
      const cur = Array.isArray(ov._workDescriptionPolishLearning)
        ? ov._workDescriptionPolishLearning
        : [];
      const next = cur.concat([{ ...entry, ts: entry.ts || Date.now() }]).slice(-200);
      await this.saveJob("_workDescriptionPolishLearning", next);
      return { ok: true };
    },
