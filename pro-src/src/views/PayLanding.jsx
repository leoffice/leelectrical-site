// Public View & Pay page — BLZ Electric branding, invoice details, View invoice PDF,
// editable amount + fee, then Pay → Sola PaymentSITE.
...
export default function PayLanding() {
  ...
  const showBillAddr = data.ba && data.ba !== data.sa;
  ...
  return (
    <div className="min-h-screen bg-[#f4f6f8]">
      <header className="bg-white border-b border-slate-200/80 px-4 py-4 pt-safe shadow-sm">
        <div className="max-w-lg mx-auto flex items-center gap-4">
          ...
          <div className="min-w-0">
            <div className="font-extrabold text-lg tracking-tight text-slate-900 leading-tight">BLZ Electric</div>
      ...
      <main className="max-w-lg mx-auto px-4 py-6 pb-10">
        <div className="card p-5 mb-4">
          <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
            <div>
              ...
              <h1 className="text-2xl font-extrabold text-slate-900">#{data.i}</h1>
            </div>
            <button
              type="button"
              className="btn-brand !py-2 !px-4 text-sm shrink-0"
          ...
          {data.c && (
            <p className="text-sm font-semibold text-slate-800 mb-3">{data.c}</p>
          )}

          <dl className="text-sm space-y-2 text-slate-600">
            {data.sa ? (
              <div>
                <dt className="text-[11px] font-bold uppercase tracking-wide text-slate-400">Service address</dt>
                <dd className="mt-0.5">{data.sa}</dd>
              </div>
            ) : null}
            {showBillAddr ? (
              <div>
                <dt className="text-[11px] font-bold uppercase tracking-wide text-slate-400">Billing address</dt>
                <dd className="mt-0.5">{data.ba}</dd>
              </div>
            ) : null}
            {!data.sa && data.ba ? (
              <div>
                <dt className="text-[11px] font-bold uppercase tracking-wide text-slate-400">Address</dt>
                <dd className="mt-0.5">{data.ba}</dd>
              </div>
            ) : null}
            {data.w ? (
              <div>
                <dt className="text-[11px] font-bold uppercase tracking-wide text-slate-400">Work</dt>
                <dd className="mt-0.5">{data.w}</dd>
              </div>
            ) : null}
          </dl>
        </div>
        ...
        <p className="text-[11px] text-slate-500 mb-4 px-1 leading-relaxed">
          Tap ✏️ to change the amount before Pay. Processing fee is 3.5% on top ({fmtMoneyPrecise(payAmount)} +{" "}
          {fmtMoneyPrecise(fee)} = <b>{fmtMoneyPrecise(chargeTotal)}</b>).
        </p>

        <a
          href={payUrl || "#"}
          className={`btn-brand w-full !py-4 text-base shadow-md mb-3 ${
        ...
        <p className="text-center text-[11px] text-slate-400 px-2">
          Card page: enter payment details only — name, address, and amount are pre-filled.
        </p>
      </main>
