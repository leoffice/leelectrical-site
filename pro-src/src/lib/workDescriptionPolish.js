...
/** Detect labor-only / materials-by-customer notes in the draft. */
export function extractCommercialNotes(raw) {
  const t = String(raw || "");
  const laborOnly =
    /labor\s+only|prices?\s+include\s+labor|pricing\s+is\s+labor|materials?\s+will\s+be\s+an\s+additional/i.test(
      t
    );
  ...
  return { laborOnly, materialsByCustomer };
}

function isCommercialBullet(chunk) {
  return /labor\s+only|all\s+prices?\s+include\s+labor|materials?\s+will\s+be|materials?\s+(?:are\s+)?(?:an\s+additional|supplied|purchased|by\s+the\s+customer)|pricing\s+is\s+labor|not\s+included\s+in\s+this\s+price|purchased\s+separately/i.test(
    chunk
  );
}
...
function invoiceBullets(raw) {
  ...
  const single = clean(text);
  if (!single || isCommercialBullet(single)) return [];
  return [single];
}
...
function professionalBody(raw, ctx) {
  ...
  const scopeLines =
    bullets.length >= 1 ? bullets.map(formatScopeBullet).filter(Boolean) : [workBody(clean(raw))].filter(Boolean);
