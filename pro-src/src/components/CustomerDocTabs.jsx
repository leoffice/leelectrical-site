...
function EstimateRows({ list, activeJobId, onOpen }) {
  if (!list.length) return null;
  return list.map((j) => {
      ...
      {tab === "estimates" ? (
        <div className="card px-3 py-2" data-testid="cust-tab-panel-estimates">
          <DocSection title="Open estimates" empty={!openEst.length}>
            <EstimateRows list={openEst} activeJobId={activeJobId} onOpen={openJob} />
          </DocSection>
          <DocSection title="Closed estimates" empty={!closedEst.length}>
            <EstimateRows list={closedEst} activeJobId={activeJobId} onOpen={openJob} />
          </DocSection>
        </div>
      ) : null}
    </div>
