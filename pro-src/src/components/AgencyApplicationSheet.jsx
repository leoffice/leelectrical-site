...
export default function AgencyApplicationSheet({ job, agencyId = "coned-form-a", onClose, onSave }) {
  ...
  const updateAnswer = (key, value) => {
    ...
    // Con Ed Part Supply unit (Account + Mailing on page 1): first auto-abbrev; second correction left alone.
    if (key === "serviceUnit" || key === "billingUnit" || key === "mailingUnit") {
      ...
      {!reviewMode && step ? (
        ...
        <div data-testid="agency-review">
          ...
          <p className="text-xs text-slate-500 mb-2">
            We fill page 1 of the official Con Ed Application for Service (Form A) and email that
            completed PDF to the office/contact — not to Con Ed. You still sign in to the Con Ed
            Energy Services portal yourself to file it. The app never enters your Con Ed password.
          </p>
          {CONED_FORM_A_SOURCE_PDF ? (
            <p className="text-[11px] text-slate-400 mb-3" data-testid="agency-source-form">
              Source form: company file application-for-service.pdf (Form A) — real fields filled, not a recreated layout.
            </p>
          ) : null}
          <Fld
