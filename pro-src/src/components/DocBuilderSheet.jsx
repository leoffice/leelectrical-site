...
export default function DocBuilderSheet({
  ...
  // Same face base as save path — empty/stale mobile lines must not zero a $ discount.
  const faceTotal = useMemo(() => docFaceTotal(job, subtotal), [job, subtotal]);
  const discBase = useMemo(() => {
    if (discountType === "percent") return subtotal > 0 ? subtotal : faceTotal;
    return faceTotal > 0 ? faceTotal : subtotal;
  }, [discountType, subtotal, faceTotal]);
  const preDiscount = subtotal > 0 ? subtotal : faceTotal;
  const discountDollars = useMemo(
    () =>
      resolveDiscountAmount(discBase, {
        type: discountType,
        value: discountValue,
      }),
    [discBase, discountType, discountValue]
  );
  const total = useMemo(
    () =>
      docTotalAfterDiscount(preDiscount, {
        type: discountType,
        value: discountValue,
      }),
    [preDiscount, discountType, discountValue]
  );
  const title =
    mode === "edit"
  ...
  const submitSync = async (send, opts = {}) => {
    ...
    try {
      ...
      if (needsCustomer && docSource === DOC_SOURCE_QBO) {
        ...
        // QBO / non-local path — close UI first (snappy). Do not await enqueue/PDF with builder open.
        // Levi 2026-08-21 RED LINE: never queue QuickBooks email after create.
        const recurNote =
        ...
        if (send && emailTo) {
          showToast(
            "Saved to QuickBooks — send the " +
              (kind === "estimate" ? "estimate" : "invoice") +
              " from LE Pro (local PDF), not through QuickBooks" +
              recurNote +
              attNote
          );
        } else {
        ...
        const runQboBg = async () => {
          await Promise.allSettled(
            commands.map((cmd, i) => {
              ...
              return enqueue(cmd.type, jobId, payload, "judgment", cmd.idk);
            })
          );
          await Promise.allSettled(
            attsForQbo.map((att) => {
        ...
        const kickQboBg = () => {
          runQboBg().catch(() => {
            showToast("QuickBooks sync hit a snag — open the document and try again");
          });
        };
        if (typeof requestAnimationFrame === "function") {
          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
              setTimeout(kickQboBg, 0);
            });
          });
        } else {
          setTimeout(kickQboBg, 0);
        ...
        <div className="flex items-center gap-1 min-w-0">
          ...
          <button
            ...
            onClick={() => {
              ...
              setDiscountType((t) => {
                const next = t === "percent" ? "amount" : "percent";
                discountTypeRef.current = next;
                return next;
              });
            }}
            aria-label={
