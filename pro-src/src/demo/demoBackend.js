...
async function route(fn, params, method, body, store) {
  switch (fn) {
    ...
    /* ---- takeoff: synthesise a worker-output document per chosen file so the
           whole attach → process → review → submit → export flow works offline.
           Counts are deterministic (seeded by filename) so a demo never drifts.
           This mirrors the shape netlify/functions/takeoff.mjs returns. ---- */
    case "takeoff": {
      if (method !== "POST" || (body && body.op && body.op !== "process")) {
        return json({ ok: true, demo: true });
      }
      const files = Array.isArray(body.files) ? body.files : [];
      const CLASSES = [
        { c: "WP", d: "Weatherproof GFI receptacle", base: 6 },
        { c: "GFI", d: "GFI receptacle", base: 9 },
        { c: "R", d: "Duplex receptacle", base: 24 },
        { c: "S3", d: "3-way switch", base: 5 },
        { c: "S", d: "Single-pole switch", base: 12 },
        { c: "EM", d: "Emergency light", base: 4 },
        { c: "EX", d: "Exit sign", base: 3 },
        { c: "DATA", d: "Data outlet", base: 16 },
        { c: "TV", d: "TV/coax outlet", base: 4 },
        { c: "J", d: "Junction box", base: 7 },
      ];
      const hash = (s) => {
        let h = 0;
      ...
      const documents = (files.length ? files : [{ name: "sheet.pdf" }]).map((f, fi) => {
        const seed = hash(String(f.name || "sheet") + fi);
        const counts = {};
        const candidates = [];
        CLASSES.forEach((k, i) => {
          // Deterministic per-file count: base ± a small seeded wobble, min 1.
          const n = Math.max(1, k.base + ((seed >> (i % 12)) % 5) - 2);
          counts[k.c] = n;
          for (let j = 0; j < n; j++) {
            candidates.push({
              sheet_id: fi + 1,
              ...
              score: 0.6,
              status: "candidate",
              confidence: i < 3 ? "supported" : "inferred",
              nearest_text: k.d,
            });
          }
        });
        return {
          engine: "demo-vector-v0",
          worker_id: `demo-${fi + 1}`,
          assignment: { source_sha256: `demo${seed.toString(16)}`, sheet_ids: [fi + 1] },
          candidates,
          summary: {
            counts,
            anomalies: [],
            notes:
              "DEMO synthetic takeoff — deterministic counts from the file name. " +
              "In production this comes from the PyMuPDF vector service (accurate) or the in-Worker text heuristic (inferred).",
          },
        };
      });
      return json({ ok: true, engine: "demo-vector-v0", documents });
    }
